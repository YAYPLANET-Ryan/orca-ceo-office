...
function Read-AsarVersion([string]$AsarPath) {
  ...
  $text = [Text.Encoding]::UTF8.GetString($bytes, 0, [Math]::Min($bytes.Length, 1048576))
  $match = [regex]::Match($text, '"version"\s*:\s*"([^"]+)"')
  if (-not $match.Success) { throw "Could not read app.asar version: $AsarPath" }
  $match.Groups[1].Value
}

function Get-ArchiveFileCount([string]$Path) {
  $sevenZip = (Get-Command 7z.exe, 7za.exe -ErrorAction SilentlyContinue | Select-Object -First 1).Source
